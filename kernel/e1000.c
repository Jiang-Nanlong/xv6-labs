...
#ifdef LAB11_NETWORK
  acquire(&e1000_lock);
  uint32 tail = regs[E1000_TDT];   // 找到尾指针找到可用的buffer索引
  tail %= TX_RING_SIZE;
  if((tx_ring[tail].status & E1000_TXD_STAT_DD) == 0){  // 说明还没有结束上一个transmit
    release(&e1000_lock);
    return -1;
  }

  if(tx_mbufs[tail])  // 如果对应的数据数组上有使用完的mbufs，则释放
